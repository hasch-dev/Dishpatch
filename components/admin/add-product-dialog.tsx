...
export default function AddProductDialog() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Snappy cleanup cycle: Refresh Server Payload, shut down modal, clear states
      router.refresh();
      setIsOpen(false);
  ...
  if (!isOpen) return (
    <Button 
      ...
      className="rounded-none bg-primary hover:bg-primary/90 text-primary-foreground font-black uppercase tracking-[0.3em] text-[10px] h-10 px-6 shadow-lg shadow-primary/20"
    >
  ...
  return (
    <div className="fixed inset-0 bg-background/90 backdrop-blur-md z-[500] flex items-center justify-center p-4">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
          ...
          <div className="space-y-6">
            <div className="space-y-1">
              ...
              <input name="name" placeholder="E.g., Private Dinner Experience" required className="w-full bg-background border border-foreground/10 p-3 text-xs font-mono focus:outline-none focus:border-primary" />
            </div>
            ...
            <div className="space-y-1">
              ...
              <input name="href" defaultValue="/" placeholder="/products/target-url" required className="w-full bg-background border border-foreground/10 p-3 text-xs font-mono focus:outline-none focus:border-primary text-primary" />
            </div>
          </div>
          ...
          <div className="space-y-6 flex flex-col">
            <div className="space-y-1 flex-1">
              ...
              <textarea name="description" placeholder="Enter detailed technical specifications or product description..." className="w-full h-[100px] bg-background border border-foreground/10 p-3 text-xs font-mono focus:outline-none focus:border-primary resize-none custom-scrollbar" />
            </div>

            <div className="space-y-1">
              <label className="text-[9px] uppercase tracking-widest font-black text-muted-foreground">Visual Schematic</label>
              <div className={`relative border-2 border-dashed p-6 text-center flex flex-col items-center justify-center ${selectedFile ? 'border-primary bg-primary/5' : 'border-foreground/10 bg-background hover:border-primary/50'}`}>
                {selectedFile ? (
          ...
          <Button 
            type="submit" 
            disabled={loading} 
            className="col-span-1 md:col-span-2 w-full rounded-none h-14 bg-primary text-primary-foreground hover:bg-primary/90 font-black uppercase tracking-[0.5em] text-xs mt-4"
          >
            {loading ? (
