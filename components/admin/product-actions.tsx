...
import { 
  ...
  PackageCheck, AlertTriangle, XCircle, Copy, ExternalLink 
} from "lucide-react";
import { 
...
export default function ProductActions({ product, onUpdate }: { product: any, onUpdate: () => void }) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56 rounded-none border-foreground/10 bg-background font-mono text-[10px] uppercase">
        ...
        <DropdownMenuItem onClick={() => updateStatus({ stock_status: 'in_stock' })} className="text-green-500">
          <PackageCheck className="mr-2 h-3 w-3" /> Set: In Stock
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => updateStatus({ stock_status: 'low_stock' })} className="text-yellow-500">
          <AlertTriangle className="mr-2 h-3 w-3" /> Set: Low Stock
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => updateStatus({ stock_status: 'out_of_stock' })} className="text-red-500">
        ...
        {/* Destructive */}
        <DropdownMenuItem onClick={deleteProduct} className="text-red-500 focus:bg-red-500 focus:text-white">
