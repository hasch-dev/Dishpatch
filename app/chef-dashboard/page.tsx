...
export default function ChefDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<'pending' | 'deals' | 'completed'>('pending')
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const { data: chefData } = await supabase
        .from('chef_details')
        .select('cuisine_types, hourly_rate')
        .eq('id', user.id)
        .single()
      ...
      if (proposalsRes.ok) {
        ...
        setProposals(data.filter((p: any) => p.chef_id === user.id))
      }

      if (dealsRes.ok) {
        const { data } = await dealsRes.json()
        setDeals(data.filter((d: any) => d.chef_id === user.id))
      }
          ...
          <div className="space-y-3">
            ...
            <Card className="border rounded-lg">
              <CardContent className="p-3">
                ...
                <p className="text-xl font-semibold">{pendingProposals.length}</p>
              </CardContent>
            </Card>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col md:flex-row gap-2 md:items-center md:justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" variant={activeTab === 'deals' ? 'default' : 'outline'} onClick={() => setActiveTab('deals')}>
                  Active ({deals.length})
                </Button>
                ...
                >
                  ...
                  {filteredData.length === 0 ? (
                    ...
                    filteredData.map((item: any) => (
                      <Card key={item.id} className="border rounded-lg hover:shadow-sm transition">
